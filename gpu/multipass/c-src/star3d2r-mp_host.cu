...
double 
kernel_stencil(SB_TYPE * A1, int compsize, int timestep, bool scop)
{
	...
	int		pj;
	int		pk;
	const char     *spi = getenv("pi");
	const char     *spj = getenv("pj");
	const char     *spk = getenv("pk");
	if (spi == NULL) {
		pi = 2;
	} else {
		pi = atoi(spi);
	...
	if (scop) {
		...
		float          *__dev_A;
		float          *dev_A;
		cudaCheckReturn(cudaMalloc((void **)&__dev_A, (size_t) (2) * (size_t) (dimsize) * (size_t) (dimsize) * (size_t) (dimsize) * sizeof(float)));

		{
			cudaCheckReturn(cudaMemcpy(__dev_A, A, (size_t) (2) * (size_t) (dimsize) * (size_t) (dimsize) * (size_t) (dimsize) * sizeof(float), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
		...
		for (int bt = 0; bt < T; bt += BT) {
			// shift dev_A pointer up and left BENCH_RAD * BT rows and columns
      dev_A = &(__dev_A[(pi*PI-BENCH_RAD*bt)*dimsize + pj*PJ-BENCH_RAD*bt])
...
#define __c0 c0
					const AN5D_TYPE	__c1Len = (min(93, dimsize - 1) - 2 + 1);
					const AN5D_TYPE	__c1Pad = (2);
#define __c1 c1
					const AN5D_TYPE	__c2Len = (min(93, dimsize - 1) - 2 + 1);
					const AN5D_TYPE	__c2Pad = (2);
#define __c2 c2
					const AN5D_TYPE	__c3Len = (min(93, dimsize - 1) - 2 + 1);
...
#define __c3 c3
		...
		cudaCheckKernel();
		{
			...
			SB_STOP_INSTRUMENTS;
#endif
			cudaCheckReturn(cudaMemcpy(A, __dev_A, (size_t) (2) * (size_t) (dimsize) * (size_t) (dimsize) * (size_t) (dimsize) * sizeof(float), cudaMemcpyDeviceToHost));
		}
		cudaCheckReturn(cudaFree(__dev_A));
	} else {

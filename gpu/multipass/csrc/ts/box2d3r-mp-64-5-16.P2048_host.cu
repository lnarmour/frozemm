#include <assert.h>
#include <stdio.h>
#include "box2d3r-mp-64-5-16.P2048_kernel.hu"
#define BENCH_DIM 2
#define BENCH_FPP 97
#define BENCH_RAD 3
...
double kernel_stencil(SB_TYPE *A1, int compsize, int timestep, bool scop)
{
  ...
  if (scop) {
    if (timestep >= 1 && p >= 0 && p <= 1048574 && dimsize >= 2048 * p + 1) {
...
#define AN5D_TYPE unsigned
      ...
      #define __c0 c0
      const AN5D_TYPE __c1Len = (min(dimsize - 1, 2048 * p + 2047) - 2048 * p + 1);
      const AN5D_TYPE __c1Pad = (2048 * p);
      #define __c1 c1
      const AN5D_TYPE __c2Len = (min(dimsize - 1, 2048 * p + 2047) - 2048 * p + 1);
      const AN5D_TYPE __c2Pad = (2048 * p);
      #define __c2 c2
      ...
      AN5D_TYPE __side0LenMax;
      {
        ...
        {
          kernel0_5<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, p, c0);
        }
      }
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
            kernel0_3<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, p, c0);
          }
          c0 += 1;
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
            kernel0_2<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, p, c0);
          }
        }
      ...
      else if (__c0Len % __side0LenMax)
      {
        ...
        else if (__c0Len % __side0LenMax == 4)
        {
          ...
          dim3 k0_dimBlock(__blockSize, 1, 1);
          dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
          kernel0_4<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, p, c0);
        }
      }
    }
    cudaCheckKernel();

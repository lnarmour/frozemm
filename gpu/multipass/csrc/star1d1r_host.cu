...
double kernel_stencil(SB_TYPE *A1, int compsize, int timestep, bool scop)
{
  ...
  if (scop) {
    if (dimsize >= 1 && timestep >= 1 && dimsize >= pi + 2 && pi >= -510 && pi <= 2147483135) {
...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_A, (size_t)(2) * (size_t)(dimsize) * sizeof(float)));
      
{
      cudaCheckReturn(cudaMemcpy(dev_A, A, (size_t)(2) * (size_t)(dimsize) * sizeof(float), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
...
#define AN5D_TYPE unsigned
      ...
      AN5D_TYPE __side0LenMax;
      {
        ...
        dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len), 1, 1);
        ...
        {
          kernel0_4<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, c0);
        }
      }
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len), 1, 1);
            kernel0_2<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, c0);
          }
          c0 += 1;
          {
            const AN5D_TYPE __side0Len = 2;
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len), 1, 1);
            kernel0_3<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, c0);
          }
          c0 += 1;
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len), 1, 1);
            kernel0_1<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, c0);
          }
          c0 += 1;
          {
            const AN5D_TYPE __side0Len = 1;
...
SB_STOP_INSTRUMENTS;
#endif
      cudaCheckReturn(cudaMemcpy(A, dev_A, (size_t)(2) * (size_t)(dimsize) * sizeof(float), cudaMemcpyDeviceToHost));
}

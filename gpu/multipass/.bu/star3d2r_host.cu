...
double kernel_stencil(SB_TYPE *A1, int compsize, int timestep, bool scop)
{
  ...
  if (scop) {
    if (timestep >= 1 && pi >= 0 && pi <= 21474835 && dimsize >= 100 * pi + 1 && pj >= 0 && pj <= 21474835 && dimsize >= 100 * pj + 1 && pk >= 0 && pk <= 21474835 && dimsize >= 100 * pk + 1) {
...
#define AN5D_TYPE unsigned
      ...
      #define __c0 c0
      const AN5D_TYPE __c1Len = (min(dimsize - 1, 100 * pi + 99) - 100 * pi + 1);
      const AN5D_TYPE __c1Pad = (100 * pi);
      #define __c1 c1
      const AN5D_TYPE __c2Len = (min(dimsize - 1, 100 * pj + 99) - 100 * pj + 1);
      const AN5D_TYPE __c2Pad = (100 * pj);
      #define __c2 c2
      const AN5D_TYPE __c3Len = (min(dimsize - 1, 100 * pk + 99) - 100 * pk + 1);
      const AN5D_TYPE __c3Pad = (100 * pk);
      #define __c3 c3
      ...
      AN5D_TYPE __side0LenMax;
      {
        ...
        {
          kernel0_2<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, pj, pk, c0);
        }
      }
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len) * ((__c3Len + __side3Len - 1) / __side3Len), 1, 1);
            kernel0_1<<<k0_dimGrid, k0_dimBlock>>> (dev_A, dimsize, timestep, pi, pj, pk, c0);
          }
          c0 += 1;
          {
            const AN5D_TYPE __side0Len = 1;
